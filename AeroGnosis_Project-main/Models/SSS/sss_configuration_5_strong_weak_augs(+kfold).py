# -*- coding: utf-8 -*-
"""sss configuration 5 strong-weak augs(+kfold).ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1r-Xcyz-D5EdfVLuLb42i8mWAloBVWwSf
...
import glob
checkpoint_dir = 'exp/aircraft_my_project_strong_weak'
...
def parse_args():
    parser = argparse.ArgumentParser(description='K-Fold + Strong-Weak Semi-Supervised Learning')
    parser.add_argument('--n-folds', type=int, default=5, help='Number of folds')
    parser.add_argument('--save-path', type=str, default='exp/aircraft_unetpp_kfold_strong_weak')
...
def get_default_config():
    return {
        ...
        'labeled_ratio': 0.7,
        'unsup_weight': 1.0,
        'consistency_weight': 1.0,  # Strong-weak consistency
...
def create_kfold_splits(data_root, n_folds=5, labeled_ratio=0.7, seed=42):
    ...
    print(f"\n Creating {n_folds}-Fold CV with AGGRESSIVE Strong-Weak Augmentation")
...
class StrongAugDataset(Dataset):
    def __init__(self, data_root, split_file, size=512):
        ...
        # AGGRESSIVE augmentation (performs better)
        self.transform = A.Compose([
...
def train_one_fold(fold_split, fold_idx, cfg, device, save_path):
    ...
    print(f"\n{'='*80}")
    print(f"TRAINING FOLD {fold_idx}/{cfg['n_folds']} (K-FOLD + AGGRESSIVE STRONG-WEAK)")
    print(f"{'='*80}\n")
    ...
    #Load datasets with STRONG-WEAK augmentation
    print(f"\nLoading datasets for fold {fold_idx}...")
    print("   Labeled: Moderate augmentation")
    print("  Unlabeled (WEAK): Minimal augmentation → Pseudo-labels")
    print("   Unlabeled (STRONG): AGGRESSIVE augmentation → Consistency\n")

    trainset_l = LabeledDataset(DATA_ROOT, os.path.join(fold_dir, 'labeled.txt'), size=cfg['crop_size'])
    trainset_u_weak = WeakAugDataset(DATA_ROOT, os.path.join(fold_dir, 'unlabeled.txt'), size=cfg['crop_size'])
    trainset_u_strong = StrongAugDataset(DATA_ROOT, os.path.join(fold_dir, 'unlabeled.txt'), size=cfg['crop_size'])
    valset = ValDataset(DATA_ROOT, os.path.join(fold_dir, 'val.txt'), size=cfg['crop_size'])
    ...
    print(f"  Labeled: {len(trainset_l)}")
    print(f"  Unlabeled: {len(trainset_u_weak)}")
    ...
    for epoch in range(cfg['epochs']):
        ...
        if (epoch + 1) % cfg['eval_interval'] == 0:
            ...
            print(f"  Train Loss: {losses_total.avg:.4f} (L: {losses_l.avg:.4f}, U: {losses_u.avg:.4f}, C: {losses_consistency.avg:.4f})")
...
def main():
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"\n K-FOLD + AGGRESSIVE STRONG-WEAK AUGMENTATION")
    ...
    print(f"\n{'='*80}")
    print(f"K-FOLD + AGGRESSIVE STRONG-WEAK RESULTS ({args.n_folds} FOLDS)")
    print(f"{'='*80}\n")
    ...
    print(f"\n{'='*80}")
    print(" K-Fold + AGGRESSIVE Strong-Weak Training Complete")
...
def plot_kfold_results(save_path='exp/aircraft_unetpp_kfold_strong_weak'):

    # Load CV summary
    summary_path = os.path.join(save_path, 'cv_summary.json')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'cv_bar_plot.png'), dpi=150, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'cv_box_plot.png'), dpi=150, bbox_inches='tight')
    plt.close()

    print(" Saved: cv_box_plot.png")

    # 3. Heatmap showing per-fold performance
    fig, ax = plt.subplots(figsize=(10, 6))

    # Create matrix: rows = folds, columns = metrics
    fold_data = []
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'cv_heatmap.png'), dpi=150, bbox_inches='tight')
    plt.close()
    ...
    plt.suptitle(f'{n_folds}-Fold Cross-Validation - Training History',
                 fontsize=16, fontweight='bold', y=1.02)
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'cv_training_history.png'), dpi=150, bbox_inches='tight')
    plt.close()
    ...
    plt.title(f'{n_folds}-Fold Cross-Validation - Statistical Summary',
              fontsize=14, fontweight='bold', pad=20)
    plt.savefig(os.path.join(save_path, 'cv_statistics_table.png'), dpi=150, bbox_inches='tight')
    plt.close()
