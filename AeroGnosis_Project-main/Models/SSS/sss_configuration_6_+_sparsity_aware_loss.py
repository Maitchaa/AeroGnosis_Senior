# -*- coding: utf-8 -*-
"""sss configuration 6 + sparsity aware loss
...
"""# Training with Sparsity Aware Loss"""
...
def compute_sparsity_aware_loss(pred_prob, device='cuda', return_components=False, threshold=0.5):
   #Compute the Sparsity-Aware Loss (LSA) from the paper

    # fixed issues:
    #1. Use proper threshold (0.5 instead of 0)
    #2. Better handling of edge cases
    #3. Debug output when regions found
    ...
    for b in range(batch_size):
        # get probability map for crack class (class 1) and apply proper threshold
        prob_map = pred_prob[b, 1, :, :].detach().cpu().numpy()

        # use 0.5 threshold instead of 0
        binary_mask = (prob_map > threshold).astype(np.uint8)

        # xompute imbalance ratio
        crack_pixels = binary_mask.sum()
        total_pixels = binary_mask.size
        background_pixels = total_pixels - crack_pixels

        #if no cracks detected at all, skip (don't penalize)
        if crack_pixels == 0:
            continue

        ir = background_pixels / crack_pixels

        # label connected regions using scipy with proper structure
        ...
        for region_id in range(1, num_regions + 1):
            ...
            #count regions with same size
            if region_size in region_size_counts:
                region_size_counts[region_size] += 1
            else:
                region_size_counts[region_size] = 1

        #compute LSA according to equation (4) from the paper
        lsa_sum = 0.0
        for region_size in region_sizes:
            m_ai = region_size_counts[region_size]
            ai = region_size

            lsa_sum += (m_ai / ai) ** 2

        #apply the IR weighting factor
    ...
    if total_regions > 0:
        ...
        # if no regions found in entire batch return penalty
        avg_lsa = 0.001
        avg_regions = 0
        avg_region_size = 0
...
def compute_lsssa_loss(pred, target, lambda_weight=0.4, device='cuda', lsa_threshold=0.5):
   # LSSSA = λ(LSA + LBCE) + (1-λ)LDice
    ...
    # Combine according to new eqn: LSSSA = λ(LSA + LBCE) + (1-λ)LDice
...
def parse_args():
    parser = argparse.ArgumentParser(description='Semi-Supervised Semantic Segmentation with LSSA')
    ...
    parser.add_argument('--save-path', type=str, default='exp/aircraft_unetpp_lssa_fixed')
...
def get_default_config():
    ...
    return {
        ...
        'lambda_weight': 0.4,
        'lsa_threshold': 0.5,  # probability threshold for region detection
        # Optimizer
        'optimizer': 'adam',
        'momentum': 0.9,
        'weight_decay': 0,
...
def main():
    ...
    print(f"\nStarting semi-supervised training with LSSA for {cfg['epochs']} epochs...")
    print(f"Lambda weight (λ): {cfg['lambda_weight']}")
    ...
    for epoch in range(cfg['epochs']):
        ...
        for i in range(num_batches):
            ...
            if cfg['mixed_precision']:
                with autocast():
                    # FIXED: Forward pass with proper LSA threshold
                    pred_l = model(img_l)
            ...
            if (i + 1) % cfg['print_interval'] == 0:
                print(f"Epoch [{epoch+1}/{cfg['epochs']}] Iter [{i+1}/{num_batches}] "
                      f"Loss: {losses_total.avg:.4f} (L: {losses_l.avg:.4f}, U: {losses_u.avg:.4f}) "
                      f"[LSA: {lsa_meter.avg:.4f}, BCE: {bce_meter.avg:.4f}, Dice: {dice_meter.avg:.4f}, Regions: {regions_meter.avg:.1f}]")
        ...
        if (epoch + 1) % cfg['eval_interval'] == 0:
            ...
            print(f"  Loss Components: LSA={lsa_meter.avg:.4f}, BCE={bce_meter.avg:.4f}, Dice={dice_meter.avg:.4f}")
            print(f"  Train Regions: {regions_meter.avg:.1f}")
            print(f"  Val Loss: {val_metrics['loss']:.4f}")
            print(f"  Val LSA: {val_metrics['lsa']:.4f}, Val Regions: {val_metrics['regions']:.1f}")
