# -*- coding: utf-8 -*-
"""sss configuration 4+(weak)strong-weakaugs.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1OP5Mo0R2uP4tvUCBgQZXzrYIkQ-MQsNN
...
class WeakAugDataset(Dataset):
        ...
        # WEAK augmentation - EXACTLY your original augmentations
        mean = [0.485, 0.456, 0.406]
        std = [0.229, 0.224, 0.225]

        self.transform = A.Compose([
            A.RandomScale(scale_limit=0.5, p=0.5),  # Your original
            A.Rotate(limit=10, p=0.5),              # Your original
            A.HorizontalFlip(p=0.5),                # Your original
            A.VerticalFlip(p=0.3),                  # Your original
            A.Resize(size, size),
    ...
    def __getitem__(self, idx):
        ...
        # Load mask (even though we won't use it for unlabeled data)
        mask_path = os.path.join(self.data_root, 'SegmentationClass', f"{img_id}.png")
        if not os.path.exists(mask_path):
            mask_path = os.path.join(self.data_root, 'SegmentationClass', f"{img_id}.jpg")
        mask = Image.open(mask_path)
...
class StrongAugDataset(Dataset):
    """
    Dataset with STRONG augmentation for unlabeled data
    Used for consistency regularization (student predictions)

    Strong augmentations = Your original + additional transforms:
    - Your original geometric transforms (same as weak)
    - PLUS: Random zoom, cutout, elastic transforms
    - PLUS: Light color jittering (subtle for cracks)
    """
    def __init__(self, data_root, split_file, size=512):
        self.data_root = data_root
        self.size = size
        ...
        # STRONG augmentation = Original + Additional
        mean = [0.485, 0.456, 0.406]
        std = [0.229, 0.224, 0.225]
        ...
        self.transform = A.Compose([
            ...
            A.CoarseDropout( max_holes=5,max_height=32, max_width=32,min_holes=1,min_height=8,min_width=8,fill_value=0,p=0.),

            A.GridDropout( ratio=0.3, unit_size_min=8, unit_size_max=16,holes_number_x=3,holes_number_y=3,p=0.2 ),

            A.OneOf([ A.GaussianBlur(blur_limit=(3, 5), p=1.0), A.MotionBlur(blur_limit=3, p=1.0),
            ], p=0.2),
...
class LabeledDataset(Dataset):
    ...
    Uses YOUR ORIGINAL augmentations (same as weak)
    """
    def __init__(self, data_root, split_file, size=512):
        self.data_root = data_root
        self.size = size
