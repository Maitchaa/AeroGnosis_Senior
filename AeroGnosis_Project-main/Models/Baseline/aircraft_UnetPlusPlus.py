...
Original file is located at
    https://colab.research.google.com/drive/1aKZq3F23625kOwfC48WQWm_yy3FHJZP9
"""
...
for i in range(3):
    ...
    print(f"🔹 Sample {i}: Image shape = {img.shape}, Mask shape = {mask.shape}, Mask unique = {mask.unique().tolist()}")
...
def visualize_overlay(dataset, index=0, alpha=0.5):
    ...
    if mask_np.max() > 0:
        mask_rgb[..., 0] = mask_np / mask_np.max()  # normalize to 0–1
    else:
        mask_rgb[..., 0] = mask_np
...
for i in range(1, 304):   # 1 → 303 inclusive
